                    ...
                    <table class="table table-bordered" id="banner-dataTable" width="100%" cellspacing="0">
                        <thead>
                            <tr>
                                <th>S.N.</th>
                                <th>Title</th>
                                @canany(['Edit Processor Model', 'Delete Processor Model'])
                                    <th>Action</th>
                                @endcanany
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            @foreach ($mdata as $datum)
                                <tr>
                                    ...
                                    <td>{{ $datum->name }}</td>
                                    <td>
                                        @can('Edit Processor Model')
                                            <a href="{{ route('pa.processor-model.edit', $datum->id) }}"
                                                class="float-left mr-1 btn btn-primary btn-sm"
                                                style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip"
                                                title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                                        @endcan
                                        @can('Delete Processor Model')
                                            <form method="POST" action="{{ route('pa.processor-model.destroy', [$datum->id]) }}">
                                                @csrf
                                                @method('delete')
