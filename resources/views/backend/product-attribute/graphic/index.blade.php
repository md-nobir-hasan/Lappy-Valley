...
@section('main-content')
    ...
    <div class="mb-4 shadow card">
        ...
        <div class="py-3 card-header d-flex justify-content-between">
            <h6 class="float-left m-0 font-weight-bold text-primary">Graphic List</h6>
            ...
            @can('Create Graphic')
                <a href="{{ route('pa.graphic.create') }}" class="float-right btn btn-primary btn-sm" data-toggle="tooltip"
                    data-placement="bottom" title="Add User"><i class="fas fa-plus"></i> Add Graphic</a>
            @endcan
        </div>
        <div class="card-body">
                    ...
                    <table class="table table-bordered table-striped" id="banner-dataTable" width="100%" cellspacing="0">
                        <thead>
                            <tr>
                                <th>S.N.</th>
                                <th>Title</th>
                                @canany(['Edit Graphic', 'Delete Graphic'])
                                    <th>Action</th>
                                @endcanany
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            @foreach ($mdata as $datum)
                                <tr>
                                    ...
                                    <td>
                                        @can('Edit Graphic')
                                            <a href="{{ route('pa.graphic.edit', $datum->id) }}"
                                                class="float-left mr-1 btn btn-primary btn-sm"
                                                style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip"
                                                title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                                        @endcan
                                        @can('Delete Graphic')
                                            <form method="POST" action="{{ route('pa.graphic.destroy', [$datum->id]) }}">
                                                @csrf
                                                @method('delete')
