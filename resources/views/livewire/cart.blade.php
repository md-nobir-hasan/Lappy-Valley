{{-- <form action="{{ route('checkout') }}" method="POST">
    @csrf --}}
<!-- ----------cart----- -->
<div id="side_cart" x-data="{ total: 0, }"
    ...
    <div
        class="w-[400px] max-sm:w-full h-[61px] bg-[#380D37] flex justify-between text-[20px] text-[#fff] items-center pl-[20px] pr-[30px]">
            ...
            <div class='w-full' id="side_cart_body">
                @if ($carts)
                    @foreach ($carts as $cart)
                        <div x-data="{
                            qty: '{{ $cart->quantity }}',
                            price: '{{ App\Http\Helper::commaRemove($cart->product->price) }}',
                            dis_price: '{{ App\Http\Helper::commaRemove($cart->product->final_price) }}',
                            subtotal: 0,
                            cp_show: true,
                            setup() {
                                ...
                                <div class="flex text-left">
                                    <p class='text-[#353535] text-[14px] font-[jost] font-[400] text-left'>
                                        <span x-text="qty"></span> x <span
                                            class='text-[16px] font-[500] text-[#DC275C]'> {{ $cart->product->final_price }}</span>
