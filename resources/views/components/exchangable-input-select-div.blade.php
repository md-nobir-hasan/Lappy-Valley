<div class="form-group">
    ...
    <div class="input-group mb-3">
        <select name="{{ $attributes['label_for'] }}_id" class="exchangable_select form-control"
            ...
            @if ($attributes['select_data'])
                @foreach ($attributes['select_data'] as $sdatum)
                    <option value="{{ $sdatum->id }}"
                        @if ($attributes['id_for_selected'])
                            @selected($sdatum->id == $attributes['id_for_selected'])
                        @else
                            @selected($sdatum->id == old($attributes['label_for'] . '_id'))
                        @endif>
        ...
        <input type="text" name="{{ $attributes['label_for'] }}_name" id="{{ $attributes['label_for'] }}"
            placeholder="Enter the {{ $attributes['label_title'] }}"
            value="{{ old($attributes['label_for'] . '_name') }}" class="form-control exchangable_input">
        ...
        <div class="input-group-append">
            ...
            <button type="button" class="btn btn-danger m-0 p-0 select_restore"><svg xmlns="http://www.w3.org/2000/svg"
